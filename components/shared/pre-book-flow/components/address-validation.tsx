...
export function AddressValidation({ value, onSelect, onChange, isInputFocused }: AddressValidationProps) {
  ...
  const fetchPostcodeSuggestions = useCallback(async (postcode: string) => {
    ...
    try {
      ...
      const placesResponse = await new Promise<google.maps.places.PlaceResult[]>((resolve, reject) => {
  ...
  const fetchSuggestions = useCallback(async () => {
    if (!value || value.length < 2 || !autocompleteService || !sessionToken || !isGoogleLoaded) return;
